...
function InboxTab({convs,myProfile,T,onOpen,onDeleteConv,onViewProfile,onMarkAllRead}){
...
function App(){
  ...
  async function handleMarkAllRead(){
    const hasUnread=convs.some(c=>(c.messages||[]).some(m=>
      m&&m.sid&&
      m.sid!==profile.id&&
      m.read!==true&&
      !String(m.dbId||"").startsWith("temp_")
    ));
    if(!hasUnread){
      pop("All messages are already read.");
      return;
    }
    const markRead=msgs=>msgs.map(m=>m.sid===profile.id?m:{...m,read:true,status:"seen"});
    setConvs(prev=>prev.map(c=>({...c,messages:markRead(c.messages||[])})));
    setActiveChat(prev=>prev?{...prev,messages:markRead(prev.messages||[])}:prev);
    await sb.from("messages")
      .update({read:true,status:"seen"})
      .eq("receiver_id",profile.id)
      .eq("read",false);
    pop("All messages marked as read ✓");
  }
  ...
  return<>
    <div style={{minHeight:"100vh",background:T.bg,color:T.txt,paddingBottom:"calc(68px + env(safe-area-inset-bottom))"}}>
      ...
      {tab==="inbox"&&<InboxTab convs={convs.map(c=>{const other=profiles.find(p=>p.id===c.oid);return other?{...c,other}:null;}).filter(Boolean)} profiles={profiles} myProfile={profile} following={following} T={T} onOpen={(conv,other)=>setActiveChat({...conv,other})} onDeleteConv={handleDeleteConv} onViewProfile={p=>pushRoute('profile',p)} onMarkAllRead={handleMarkAllRead}/>}
