...
(function(){
  ...
  // ── INBOX ────────────────────────────────────────────────────────────────────
  function Inbox({convs,myProfile,T,onOpen,onDelete,onViewProfile,onMarkAllRead}){
    ...
    return(
      <div>
        <div style={{display:"flex",justifyContent:"flex-end",padding:"6px 16px",position:"relative"}}>
          <button onClick={()=>setShowHeaderMenu(m=>!m)} style={{background:"none",border:"none",color:T.mu,fontSize:18,padding:"4px 8px"}}>···</button>
          {showHeaderMenu&&(
            <>
              <div onClick={()=>setShowHeaderMenu(false)} style={{position:"fixed",inset:0,zIndex:9}}/>
              <div style={{position:"absolute",right:16,top:36,background:T.card,borderRadius:12,boxShadow:"0 4px 24px rgba(0,0,0,.15)",zIndex:10,overflow:"hidden",minWidth:180}}>
                <button onClick={()=>{
                  setShowHeaderMenu(false);
                  if(window.confirm("Mark all conversations as read?\n\nThis will mark every unread message across all conversations as read.")){
                    onMarkAllRead&&onMarkAllRead();
                  }
                }} style={{display:"block",width:"100%",padding:"12px 16px",textAlign:"left",background:"none",border:"none",fontSize:14,color:T.txt}}>Mark All as Read</button>
              </div>
            </>
          )}
        </div>
