...
(function(){
  ...
  function ChatScreen({conv,myProfile,profiles,T,onBack,onSend,onMarkRead,onClearChat,onBlockUser,onReportUser,onViewProfile}){
    ...
    useEffect(()=>{
      if(conv?.id){
        console.log("[HE_DEBUG][chat-opened] cid:",conv.id,"| other:",other?.id,"| reason: conversation opened");
    ...
    useEffect(()=>{
      if(conv?.id&&msgs.some(m=>m.sid!==myProfile.id&&m.read===false)){
        console.log("[HE_DEBUG][chat-new-unread] cid:",conv.id,"| reason: new unread message arrived while chat open");
        onMarkRead(conv.id,other?.id);
      }
    ...
    // Visibility re-mark
    useEffect(()=>{
      function onVis(){if(document.visibilityState==="visible"&&conv?.id){console.log("[HE_DEBUG][chat-visibility-resume] cid:",conv.id,"| reason: tab/app became visible again");onMarkRead(conv.id,other?.id);}}
  ...
  function ConvRow({conv,myProfile,T,onClick,onViewProfile,onDelete}){
    ...
    if(unread>0)console.log("[HE_DEBUG][convrow-unread] cid:",conv.id,"| other:",other.id,"| row unread:",unread,"| (independent per-row calc — should match this conv's contribution to the tab-bar badge)");
